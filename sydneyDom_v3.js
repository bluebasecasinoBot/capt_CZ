            ...
            visualTreeParams['root'] = root
                visualTreeParams['ws'].addEventListener('open', e =>{
                    if(visualTreeParams.mounted && !visualTreeParams.sent && Object.keys(sydDOM).length > 0)
                    {
                        ...
                        const funcNames = [];
                        const funcs = Object.keys(sydDOM).map(dom => {
                            funcNames.push(dom)
                            return sydDOM[dom].toString()
                        });
                        //Send function names and templates
                        ...
                        visualTreeParams['ws'].send(JSON.stringify({header:'visualTree',data:{funcRaw:funcs,funcNames,root,sC,sCNames,sydvar,varNames,mountFunc:mountFunc["fnct"].toString(),domain:location.href , cssLinks}}));
                    }
                })  
    }
}
